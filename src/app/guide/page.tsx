...
export default function GuidePage() {
  ...
  }, [weather, settings.water_clarity]);
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 flex flex-col" style={{ height: "calc(100dvh - 4rem)" }}>
      ...
      <div className="flex items-center justify-between mb-4 flex-shrink-0">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            {snapshot ? "Context loaded from current conditions" : "No conditions loaded — set location for best results"}
          </p>
        </div>
        {messages.length > 0 && (
          <button
      ...
      {error?.includes("not configured") && (
        <div className="glass-card p-4 mb-4 border border-yellow-500/20 flex-shrink-0">
          <p className="text-sm text-yellow-400 font-semibold">AI Guide Not Configured</p>
          <p className="text-xs text-slate-400 mt-1">
            Add your <code className="text-neon-cyan">ANTHROPIC_API_KEY</code> to{" "}
            <code className="text-slate-300">.env.local</code> to enable the AI guide.
          </p>
        </div>
      )}
